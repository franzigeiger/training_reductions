...
class BaseModelPool(UniqueKeyDict):
    ...
    def __init__(self):
        ...
        for model in ['alexnet', 'densenet169', 'resnet101']:
            ...
            self.add_model_to_maps(model,'_norm_dist',lambda bound_func=model_func: bound_func('norm_dist', True, apply_norm_dist))
            self.add_model_to_maps(model,'_jumbler',lambda bound_func=model_func: bound_func('jumbler', True, apply_all_jumbler))
            self.add_model_to_maps(model,'_kernel_jumbler',lambda bound_func=model_func: bound_func('kernel_jumbler', True, apply_in_kernel_jumbler))
            self.add_model_to_maps(model,'_channel_jumbler',lambda bound_func=model_func: bound_func('channel_jumbler', True, apply_channel_jumbler))
            self.add_model_to_maps(model,'_norm_dist_kernel',lambda bound_func=model_func: bound_func('norm_dist_kernel', True, apply_norm_dist_kernel))
...
brain_models = {
    ...
    'CORnet-S_norm_dist_kernel': LazyLoad(lambda: cornet_s_brainmodel('norm_dist_kernel', True, apply_norm_dist_kernel)),
}
