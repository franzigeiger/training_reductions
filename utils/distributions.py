    ...
    if path.exists(f'{base_dir}/gm_{name}_samples.pkl'):
        ...
        if components != 0:
            gmm = mixture.GaussianMixture(n_components=components,
                                          covariance_type='full', max_iter=5000, tol=1e-15,
                                          n_init=20)
            gmm.fit(param)
...
def best_fit_distribution(data, bins=200, ax=None):
    ...
    DISTRIBUTIONS = [
        ...
        st.cosine, st.dgamma, st.chi, st.chi2, st.cauchy,
        # st.dweibull, st.erlang, st.frechet_r, st.frechet_l,
        st.expon, st.exponnorm, st.exponpow,  # st.f, st.fatiguelife,
        st.foldcauchy, st.foldnorm,  # st.genlogistic, st.genpareto, st.gennorm, st.genexpon,
        st.genextreme, st.gamma,
        ...
        # st.lomax, st.maxwell, st.mielke, st.nakagami, st.ncx2,
        # st.ncf,
...
def make_pdf(dist, params, size=10000):
    ...
    start = dist.ppf(0.01, *arg, loc=loc, scale=scale) if arg else dist.ppf(0.01, loc=loc,
                                                                            scale=scale)
    ...
    for k, v in state_dict.items():
        if 'running' in k:
            if path.exists(f'{dir}rersources/{k}_weights.pkl'):
                ...
                print(
                    f'{k} Mean {mu} std {std} min {weights.min()} max {weights.max()}, shape {weights.shape}')
