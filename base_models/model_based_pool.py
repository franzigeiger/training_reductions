    ...
    for name, m in model.named_modules():
        if type(m) == nn.Conv2d:
            ...
            brain_models[f'CORnet-S_norm_dist_L{layer_number}'] = LazyLoad(
                lambda layer_number=layer_number: cornet_s_brainmodel(f'norm_dist_L{layer_number}',
                                                                      True,
                                                                      function=apply_norm_dist,
                                                                      config=[layer_number],
                                                                      type='model'))
            brain_models[f'CORnet-S_jumbler_L{layer_number}'] = LazyLoad(
                lambda layer_number=layer_number: cornet_s_brainmodel(f'jumbler_L{layer_number}',
                                                                      True,
                                                                      function=apply_all_jumbler,
                                                                      config=[layer_number],
                                                                      type='model'))
            brain_models[f'CORnet-S_fixed_value_L{layer_number}'] = LazyLoad(
                lambda layer_number=layer_number: cornet_s_brainmodel(
                    f'fixed_value_L{layer_number}', True,
                    function=apply_fixed_value, config=[layer_number],
                    type='model'))
            brain_models[f'CORnet-S_fixed_value_small_L{layer_number}'] = LazyLoad(
                lambda layer_number=layer_number: cornet_s_brainmodel(
                    f'fixed_value_small_L{layer_number}', True,
