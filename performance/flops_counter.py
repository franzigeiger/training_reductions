...
def print_model_with_flops(model, total_flops, total_params, units='GMac',
    ...
    def flops_repr(self):
        ...
        if total_flops != 0:
            flops = accumulated_flops_cost / total_flops
        else:
            flops = 0
...
def conv_flops_counter_hook(conv_module, input, output):
    if not conv_module.requires_grad:
        conv_module.__flops__
    # Can have multiple inputs, getting the first one
    input = input[0]
...
def add_flops_counter_hook_function(module):
    #  and hasattr(module, 'requires_grad') and module.requires_grad
    if is_supported_instance(module):
